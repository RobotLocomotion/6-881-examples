...
def InverseKinPointwise(p_WQ_start, p_WQ_end, duration,
                        ...
                        is_printing=True):
    """
    Calculates a joint space trajectory for iiwa by repeatedly calling IK. The first IK is
        initialized (seeded) with q_initial_guess. Subsequent IKs are seeded with the
        solution from the previous IK.

    Positions for point Q (p_EQ) and orientations for the end effector, generated
        respectively by InterpolatePosition and InterpolateOrientation, are added to
        the IKs as constraints.
    ...
    @param InterpolatePosition: A function with signature (start, end, num_knot_points, i).
        It returns p_WQ, a (3,) numpy array which describes the desired position of Q
        at knot point i in world frame.
    @param InterpolateOrientation: A function with signature

    @param p_BQ: numpy array of shape (3,), a fixed point in frame B (usually the EE
        frame) coincident with p_WQ.
    ...
    @return: qtraj: a 7-dimensional cubic polynomial that describes a
        joint-space trajectory for the iiwa arm.
    @return: q_knots: a (n, num_knot_points) numpy array (where n = plant.num_positions())
        that stores solutions returned by all IKs. It can be used to initialize IKs for
        the next trajectory.
    """
...
def GenerateOpenLeftDoorPlansByTrajectory(is_printing=True):
    ...
    The pulling actions are a result of following a joint space trajectory generated by
    solving IKs.
    """
